...
#####################################################################################################
#
# RID + Rate 2 GPS 将移动率rate转为gps坐标
#
#####################################################################################################
...
def get_rid_grid(mbr, grid_size, rn_dict):
    ...
    # 获得最大的索引号 x，y
    max_xid = int(abs(mbr.max_lat - mbr.min_lat) / lat_unit) + 1
    max_yid = int(abs(mbr.max_lng - mbr.min_lng) / lng_unit) + 1

    grid_rn_dict = {}

    # 遍历路网字典 字典条目：
    # "0": {"coords": [[39.2031381, -8.7930774], [39.2022991, -8.7949821]], "length": 188.78412711368057, "level": 2
    for k, v in rn_dict.items():
        # eid=k时的起点
        pre_lat = v['coords'][0].lat
        pre_lng = v['coords'][0].lng
        # 计算第一个点所在的网格
        pre_locgrid_x = max(1, int(abs(pre_lat - mbr.min_lat) / lat_unit) + 1)
        pre_locgrid_y = max(1, int(abs(pre_lng - mbr.min_lng) / lng_unit) + 1)
        ...
        for coord in v['coords'][1:]:
            lat = coord.lat
            lng = coord.lng
            locgrid_x = max(1, int(abs(lat - mbr.min_lat) / lat_unit) + 1)
            locgrid_y = max(1, int(abs(lng - mbr.min_lng) / lng_unit) + 1)
...
def get_dis_prob_vec(gps, rn, parameters):
    ...
    if candis is not None:
        for candi_pt in candis:
            ...
            prob = exp_prob(parameters.beta, candi_pt.error)
            cons_vec[candi_pt.eid] = prob
    else:
        cons_vec = torch.ones(parameters.id_size)
    return cons_vec

def get_constraint_mask(src_grid_seqs, src_gps_seqs, src_lengths, trg_lengths, grid_rn_dict, rn, parameters):
    ...
    for bs in range(batch_size):
        ...
        for i in range(2, src_lengths[bs]):
            ...
            # middle src gps dis_prob_mask_flag--false
            if parameters.dis_prob_mask_flag:
                cons_vec = get_dis_prob_vec(cur_gps, rn, parameters)
